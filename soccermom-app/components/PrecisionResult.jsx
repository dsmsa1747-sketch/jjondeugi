"use client";
// 정밀분석 결과화면 — 작업 상태를 폴링하고, 완료되면 추적영상 + 지표를 표시.
// analysis_mode === "match" 이면 TeamComparison(팀 비교)도 렌더.
// analysis_mode === "practice" 이면 팀 점유율 대신 개인 분석 안내 표시.
import { useEffect, useState } from "react";
import TeamComparison from "@/components/TeamComparison";

export default function PrecisionResult({ jobId }) {
  const [data, setData] = useState(null);
  const [err, setErr] = useState(null);

  useEffect(() => {
    if (!jobId) return;
    let timer;
    const poll = async () => {
      try {
        const r = await fetch(`/api/analyze-precise/status/${jobId}`);
        const j = await r.json();
        if (j.error) throw new Error(j.error);
        setData(j);
        if (j.status === "done" || j.status === "failed") return; // 폴링 종료
      } catch (e) {
        setErr(String(e.message || e));
      }
      timer = setTimeout(poll, 3000); // 3초마다 확인
    };
    poll();
    return () => clearTimeout(timer);
  }, [jobId]);

  if (err) return <p style={{ color: "crimson" }}>오류: {err}</p>;
  if (!data) return <p>불러오는 중…</p>;

  if (data.status === "queued" || data.status === "processing") {
    const pct = data.progress != null ? Math.round(data.progress * 100) : null;
    return (
      <div style={{ padding: "20px 0" }}>
        <p style={{ fontSize: 16 }}>🎬 AI가 영상을 정밀 분석 중입니다… {pct != null ? `${pct}%` : ""}</p>
        <p style={{ color: "#888" }}>영상 길이에 따라 수 분 걸릴 수 있어요. 이 화면은 자동 갱신됩니다.</p>
        <div style={{ width: "100%", background: "#f0f0f0", borderRadius: 4, height: 8, marginTop: 12 }}>
          <div
            style={{
              width: `${pct ?? 5}%`,
              height: "100%",
              background: "#22c55e",
              borderRadius: 4,
              transition: "width 0.5s",
            }}
          />
        </div>
      </div>
    );
  }

  if (data.status === "failed") {
    return <p style={{ color: "crimson" }}>분석 실패: {data.error || "알 수 없는 오류"}</p>;
  }

  // done
  const result = data.result || {};
  const players = result.players || [];
  const target = result.target_player;
  const poss = result.possession || {};
  const analysisMode = result.analysis_mode || "match";
  const isMatchMode = analysisMode === "match";

  return (
    <div>
      <h2 style={{ marginBottom: 16 }}>정밀분석 결과</h2>

      {/* 축구 영상이 아닐 가능성 경고 (선수 거의 미감지) */}
      {result.is_soccer === false && (
        <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 8, padding: 12, marginBottom: 16, color: "#b91c1c" }}>
          ⚠️ {result.warning || "축구 영상으로 보이지 않아 분석 신뢰도가 낮습니다."}
        </div>
      )}

      {data.videoUrl && (
        <video src={data.videoUrl} controls style={{ width: "100%", maxWidth: 720, borderRadius: 8, marginBottom: 16 }} />
      )}

      {/* 공식경기: 팀 비교 컴포넌트 표시 */}
      {isMatchMode && result.teams && (
        <TeamComparison teams={result.teams} />
      )}

      {/* 연습경기: 팀 구분 불가 안내 */}
      {!isMatchMode && (
        <p style={{ color: "#6b7280", fontSize: 14, margin: "8px 0", padding: "8px 12px", background: "#f9fafb", borderRadius: 6 }}>
          🏃 연습경기 모드: 팀 구분 없이 개인 분석
        </p>
      )}

      {/* 볼 점유율은 공식경기(팀 비교 없는 경우 포함)에만 표시 */}
      {isMatchMode && !result.teams && (
        <p>⚽ 볼 점유율 — A팀 {poss.team1 ?? "-"}% / B팀 {poss.team2 ?? "-"}%</p>
      )}

      {target && (
        <div style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8, margin: "12px 0", background: "#fffbeb" }}>
          <h3 style={{ margin: "0 0 8px" }}>
            ⭐ 지정 선수
            {target.name ? ` — ${target.name}` : ""}
            {target.number ? ` (#${target.number})` : ` (#${target.track_id})`}
          </h3>
          <p style={{ margin: 0 }}>
            이동거리 {target.distance} {target.unit_distance} · 평균속도 {target.avg_speed} {target.unit_speed} · 최고속도 {target.max_speed} {target.unit_speed}
          </p>
        </div>
      )}

      {players.length > 0 && (
        <>
          <h3>선수별 활동량 (이동거리 순)</h3>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ background: "#f3f4f6" }}>
                  <th style={{ padding: "8px 12px", textAlign: "left" }}>선수</th>
                  <th style={{ padding: "8px 12px", textAlign: "left" }}>팀</th>
                  <th style={{ padding: "8px 12px", textAlign: "right" }}>이동거리</th>
                  <th style={{ padding: "8px 12px", textAlign: "right" }}>평균속도</th>
                  <th style={{ padding: "8px 12px", textAlign: "right" }}>최고속도</th>
                </tr>
              </thead>
              <tbody>
                {players.slice(0, 12).map((p) => (
                  <tr key={p.track_id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                    <td style={{ padding: "8px 12px" }}>#{p.track_id}</td>
                    <td style={{ padding: "8px 12px" }}>{p.team || "-"}</td>
                    <td style={{ padding: "8px 12px", textAlign: "right" }}>{p.distance} {p.unit_distance}</td>
                    <td style={{ padding: "8px 12px", textAlign: "right" }}>{p.avg_speed} {p.unit_speed}</td>
                    <td style={{ padding: "8px 12px", textAlign: "right" }}>{p.max_speed} {p.unit_speed}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </>
      )}

      {/* 정직 라벨 — 항상 표기 */}
      <p style={{ color: "#888", fontSize: 13, marginTop: 16, padding: "8px 12px", background: "#f9fafb", borderRadius: 6, borderLeft: "3px solid #d1d5db" }}>
        ※ {result.disclaimer || "AI 측정값으로 오차가 있을 수 있습니다. 경기장 보정점이 없으면 거리/속도는 상대 추정값(px 기준)입니다."}
      </p>
    </div>
  );
}
