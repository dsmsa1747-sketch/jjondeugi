...
def get_sheets_service():
    ...
    우선순위: drive_token.json (OAuth 갱신 토큰) → credentials.json (서비스 계정 or OAuth 클라이언트)
    """
    ...
    # 1) OAuth 토큰 파일 (drive_token.json)
    if TOKEN_FILE.exists():
        try:
            ...
            print(f"[sheets_service] OAuth 토큰 로드/갱신 실패, 서비스 계정으로 시도: {e}")
            creds = None

    # 2) credentials.json — 서비스 계정 또는 OAuth 클라이언트 형식
    if creds is None and CREDS_FILE.exists():
        try:
            ...
            if info.get("type") == "service_account":
                ...
                # OAuth 클라이언트 JSON — 로컬 서버 플로우로 인증
    ...
    if creds is None:
        raise RuntimeError(
            f"인증 파일이 없습니다. '{TOKEN_FILE}' 또는 '{CREDS_FILE}'가 필요합니다."
        )
...
def _col_letter(col_idx: int) -> str:
    """0-based 컬럼 인덱스를 엑셀 열 문자로 변환 (예: 0→A, 26→AA)"""
    letter = ""
    n = col_idx
    while True:
...
def _cell(col_name: str, row_number: int) -> str:
    """컬럼 이름과 1-based 행 번호로 A1 표기법 반환 (예: B3)"""
    return f"{_col_letter(COL[col_name])}{row_number}"
...
def read_pending_products(sheet_id: str) -> list:
    ...
    for sheet_idx, row in enumerate(rows):
        ...
        product["sheet_row_index"] = sheet_idx + 1  # 1-based (헤더=1, 첫 데이터=2)
...
def update_product_result(
    ...
    telegram_sent: "발송완료" (기본값)
    """
    service = get_sheets_service()

    updates = [
        {"range": _cell("상태",       row_index), "values": [[status]]},
        {"range": _cell("처리결과",   row_index), "values": [[result]]},
        {"range": _cell("완료시간",   row_index), "values": [[completed_at]]},
        {"range": _cell("텔레그램알림", row_index), "values": [[telegram_sent]]},
    ]
